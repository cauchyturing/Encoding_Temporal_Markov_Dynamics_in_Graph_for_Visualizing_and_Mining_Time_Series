...
def QVeq(series, Q):
    ...
    dic = dict(zip(set(series), q.codes.tolist()))
    qv = np.zeros([1, Q])
    label = []
    for each in series:
...
def QM(series, Q):
    q = pd.qcut(series, Q)
    MSM = np.zeros([Q, Q])
    for i in range(0, q.codes.size - 1):
        MSM[q.codes[i]][q.codes[i + 1]] += 1
    for i in range(Q):
...
def writeQM(qm, path):
    ...
    f.write('\n')
    for i in range(size):
        f.write(str(i) + ';' + ';'.join(list(map(str, list(qm[i][:])[1:-1]))) + '\n')
    f.close()
...
def writeQM2pkl(qm, label, path, tr, te, va):
    ...
    traindata = np.array(pos[:int(len(pos) * tr)] + neg[:int(len(neg) * tr)], \
                         dtype='float32')
    testdata = np.array(
        pos[int(len(pos) * tr):int(len(pos) * (tr + te))] + neg[int(len(neg) * tr):int(len(neg) * (tr + te))], \
        dtype='float32')
    ...
    trainlabel = np.array(posl[:int(len(posl) * tr)] + negl[:int(len(negl) * tr)], \
                          dtype='float32')
    testlabel = np.array(
        posl[int(len(posl) * tr):int(len(posl) * (tr + te))] + negl[int(len(negl) * tr):int(len(negl) * (tr + te))], \
        dtype='float32')
